...
class ChordEventsTest {
    ...
    private static ChordEvent event(double start, double end, Chord chord, float first) {
        float[] chroma = new float[12];
        chroma[0] = first;
        return new ChordEvent(BigDecimal.valueOf(start), BigDecimal.valueOf(end), chord, chroma, null);
    }

    @Test
    void mergesIdenticalNeighboursWeightingChromaByDuration() {
        Chord am = Chord.of(9, ChordQuality.MIN);
        List<ChordEvent> merged = ChordEvents.mergeConsecutive(List.of(
                event(0, 1, am, 1.0f), event(1, 4, am, 0.0f), event(4, 5, Chord.of(5, ChordQuality.MAJ), 0.5f)));
    ...
    void differentBassKeepsSegmentsApart() {
        ...
        assertThat(ChordEvents.mergeConsecutive(List.of(event(0, 1, am, 1), event(1, 2, amOverGs, 1)))).hasSize(2);
    }

    @Test
    void noChordSegmentsMergeToo() {
        assertThat(ChordEvents.mergeConsecutive(List.of(
                event(0, 1, Chord.noChord(), 0), event(1, 2, Chord.noChord(), 0)))).hasSize(1);
    }
}
